...
(
    id          INTEGER GENERATED BY DEFAULT AS IDENTITY
        CONSTRAINT water_level_pk
            PRIMARY KEY,
    level DOUBLE PRECISION,
    device_id   INTEGER                 NOT NULL
        CONSTRAINT water_level_devices_id_fk
            REFERENCES devices
            ON UPDATE CASCADE ON DELETE CASCADE,
    room_id     INTEGER
        CONSTRAINT water_level_rooms_id_fk
            REFERENCES rooms
            ON UPDATE CASCADE ON DELETE CASCADE,
    relevance   TIMESTAMP DEFAULT NOW() NOT NULL
);
...
--accurate_temperature table
CREATE TABLE accurate_temperature_history
(
    id          INTEGER GENERATED BY DEFAULT AS IDENTITY
        CONSTRAINT accurate_temperature_history_pk
            PRIMARY KEY,
    temperature DOUBLE PRECISION,
    device_id   INTEGER                 NOT NULL
        CONSTRAINT accurate_temperature_devices_id_fk
            REFERENCES devices
            ON UPDATE CASCADE ON DELETE CASCADE,
    room_id     INTEGER
        CONSTRAINT accurate_temperature_rooms_id_fk
            REFERENCES rooms
            ON UPDATE CASCADE ON DELETE CASCADE,
    relevance   TIMESTAMP DEFAULT NOW() NOT NULL
);
